...
const RenderIcon = ({ icon, style, ...props }) => {
  ...
  return (
    (icon && (
      <Icon className='chip-icon' styles={style} {...props} {...icon} />
    )) ||
    null
  )
}
...
const RenderText = ({ style, text, ...props }) => {
  return (
    (text && (
      <Text className='chip-text' style={style} {...props}>
        {text}
      </Text>
    )) ||
    null
...
export const Chip = props => {
  ...
  const [chipRef, chipStyles] = useThemeHover(
    mergeStyles.default,
    mergeStyles.hover
  )
  ...
  return (
    ...
    >
      {children || [
        ...
        text &&
          renderCustomOrDefault(components.text, RenderText, {
            key: 'chip-text',
            text,
